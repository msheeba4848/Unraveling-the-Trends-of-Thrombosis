...
### 1. ANA 
When we look at the ANA dataset, which is the dataset for the antinuclear patterns, we see that the ana patterns. However, the we changed the datatype into
...
Before dropping the severity, it is important to understand the base level information and understand the association for thrombosis and the severity of it for patients who whom it is diagnosed. Looking at this, we can see that for instances when the thrombosis is present, majority of the patients suffered from 'most severe' thrombosis, followed by 'severe' and then 'mild'.

```{python}

#| label: Association between Severity and Thrombosis
#| fig-cap: "There is a high positive association between New cases recorded between 2006 and 2008 for the ages between 26-36 years. (Source: Seabold & Perktold, 2010)"
...
Though we see the association between the severity and thrombosis, it may not give accurate information since majority of data for 'severity' is missing. However, this serves as a good base to understand with the existing data.
...
# Group by 'diagnosis' and 'thrombosis', and count the occurrences
plt.figure(figsize=(6, 4))
diag_throm = diag_exam.groupby(['diagnosis', 'thrombosis']).size().unstack(fill_value=0)

# Reset the index to make 'diagnosis' a regular column
...
After looking at the female patients closely, we see that it is the Central Nervous System Lupus which is the most common symtom. The above graph talks about the female specific symptom but further exploration into general filtering based on Females, Central Nervous Lupus and Thrombosis, we can say that there are so specific symtoms that are gender related.
...
We have looked into gender in terms of symptoms and in terms of hospital admission, but how does gender play a role in terms of diagnosis? We see that for most of the female patients, we can see that for the females, most common diagnosis was 'Mixed Connective Tissue Disease', 'Other', 'Systematic Lupus Erythmatosus', 'Paget-Schotter Syndrome', 'Stevens-Johnson Syndrome'

```{python}

plt.figure(figsize=(10, 8))
sns.swarmplot(data=female_symp, x='diagnosis')
plt.title('Swarm Plot of Diagnoses Reported by Female Patients')
plt.xlabel('Diagnosis')
...
For me, another interesting aspect to explore is the diagnosis and the presence of thrombosis. Firstly, to understand the relationship between Examiation date and Thrombosis, we see there is no relationship between the Examination Date and Thrombosis, showcasing no signs of temporal trends. 

```{python}

diag_exam['thrombosis'] = diag_exam['thrombosis'].fillna(0)  
plt.figure(figsize=(8, 6))
sns.scatterplot(data=diag_exam, x='examination_date', y='thrombosis')
...
For exploring thrombosis, we need to understand the general admission status and using a pie chart, we see that around 60% of the people have not been admitted. 
...
1. You don't always need to drop all the values in the dataset unless and until it is require
2. 